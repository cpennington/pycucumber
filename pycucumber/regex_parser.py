...
class Repetition(object):
    ...
    def __str__(self):
        return self.__unicode__().encode('ascii', 'backslashreplace')
    def __unicode__(self):
        return u"%s%s" % (self.term, self.operator)
    def accept(self, visitor):
...
class Alternation(object):
    ...
    def __str__(self):
        return self.__unicode__().encode('ascii', 'backslashreplace')
    def __unicode__(self):
        return u"|".join((unicode(option) for option in self.options))
    def accept(self, visitor):
...
class TreePrinter(object):
    ...
    def visitNode(self, node):
        ret = self.indent*self.indent_count + unicode(node.__class__.__name__) + "\n"
        self.indent_count += 1
